...
> 커버하는 커밋: `보안 자격증명 분리`, `GlobalExceptionHandler`
...
이 파일이 GitHub에 올라가면 DB 비밀번호와 JWT 서명 키가 공개됩니다.
실제로 GitHub에서 `spring.datasource.password=` 를 검색하면 수천 개의 저장소가 나옵니다.
공격자가 이 정보로 DB에 직접 접근하거나, JWT를 위조할 수 있습니다.
...
`DB_PASSWORD`처럼 기본값이 없으면 환경변수가 없을 때 앱 실행이 실패합니다 — 의도된 설계입니다.
...
```properties
spring.config.import=optional:classpath:application-local.properties,optional:classpath:api.properties
```
...
핵심은 **비밀 값은 코드 저장소 밖에 있어야 한다**는 것입니다.
...
// 다른 컨트롤러: 예외를 그냥 던짐 (Spring이 500으로 처리)
@GetMapping("/recipes/{id}")
public Recipe getRecipe(@PathVariable Long id) {
    return recipeService.getRecipeById(id); // Optional.orElseThrow 예외 그대로 전파
}
```

문제점:
- 에러 응답 포맷이 컨트롤러마다 다름 (String vs JSON vs 아무것도 없음)
- 같은 예외인데 컨트롤러마다 다른 HTTP 상태코드가 나올 수 있음
- 예외처리 코드가 비즈니스 로직과 섞임

### 개념: `@RestControllerAdvice` 와 `@ExceptionHandler`

`@RestControllerAdvice`는 모든 `@Controller`에서 발생하는 예외를 한 곳에서 처리하는 AOP 기반의 어드바이스입니다.
...
`@JsonInclude(NON_NULL)`을 붙인 이유는 null 필드를 응답에서 제외하기 위해서입니다.
...
`code` 필드가 있으면 프론트엔드에서 `if (error.code === 'NOT_FOUND')` 처럼 조건 분기할 수 있습니다.
`message`만 있으면 국제화나 에러 분류가 어렵습니다.
...
**After:** 서비스에서 예외만 던지면 GlobalExceptionHandler가 알아서 처리
```java
// 서비스
