...
10개 커밋 외에, API URL 설계(RESTful 리팩토링)도 함께 진행했습니다.

| 작업 | 변경 전 | 변경 후 |
|------|---------|---------|
| RESTful API 설계 | `/deleteXxx`, `/like-toggle`, `/all` 동사 난무 | URL은 명사, 행위는 HTTP 메서드로 분리 |
...
```
RESTful 설계 → 보안 → 로깅 → 트랜잭션 → 검증 → 도메인 → 구조 → 테스트
```
