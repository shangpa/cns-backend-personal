package com.example.cns.pantry;
...
public interface PantryStockRepository extends JpaRepository<PantryStock, Long> {
    ...
    @Query("""
    select
      sum(case when s.storage = com.example.cns.pantry.StorageLocation.FRIDGE  then 1 else 0 end),
      sum(case when s.storage = com.example.cns.pantry.StorageLocation.FREEZER then 1 else 0 end),
      sum(case when s.storage = com.example.cns.pantry.StorageLocation.PANTRY  then 1 else 0 end)
    from PantryStock s
