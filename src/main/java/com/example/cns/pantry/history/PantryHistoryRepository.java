...
public interface PantryHistoryRepository extends JpaRepository<PantryHistory, Long> {
    ...
    select count(h)
     ...
     where h.action = com.example.cns.pantry.history.HistoryAction.USE
    ...
    select count(h)
     ...
     where h.pantry.id = :pantryId
       ...
       and h.stock.expiresAt is not null
       and (
            (h.action = com.example.cns.pantry.history.HistoryAction.USE     and h.createdAt > h.stock.expiresAt)
         or (h.action = com.example.cns.pantry.history.HistoryAction.DISCARD and h.createdAt > h.stock.expiresAt)
       )
    """)
    ...
    select h.ingredient.nameKo as name, sum((-1) * h.changeQty) as total
      from PantryHistory h
     where h.pantry.id = :pantryId
       and h.action = com.example.cns.pantry.history.HistoryAction.USE
       and h.createdAt between :from and :to
