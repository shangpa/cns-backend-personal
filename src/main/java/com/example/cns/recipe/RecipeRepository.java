...
import com.example.cns.admin.dto.RecipeListAdminDTO;
import com.example.cns.admin.dto.RecipeMonthlyStatsDTO;
import com.example.cns.admin.dto.UserRecipeSimpleDTO;
...
public interface RecipeRepository extends JpaRepository<Recipe, Long> {
    ...
    @Query("SELECT new com.example.cns.admin.dto.RecipeMonthlyStatsDTO(CAST(FUNCTION('DATE_FORMAT', r.createdAt, '%Y-%m') AS string), COUNT(r)) " +
            "FROM Recipe r " +
...
""")
    ...
    @Query("SELECT new com.example.cns.admin.dto.UserRecipeSimpleDTO(r.recipeId, u.username, r.title, r.createdAt) " +
    ...
    @Query("SELECT new com.example.cns.admin.dto.RecipeListAdminDTO(r.recipeId, r.user.username, r.title, r.createdAt) " +
    ...
    @Query("SELECT new com.example.cns.admin.dto.UserRecipeSimpleDTO(r.recipeId,r.user.username, r.title, r.createdAt) " +
    ...
    //레시피 검색 - 제철 음식 추천
    @Query("""
    select new com.example.cns.search.SeasonalRecipeDto(r.recipeId, r.title, r.mainImageUrl)
    from Recipe r
    where r.isPublic = true and r.isDraft = false and r.title in :titles
    """)
    List<com.example.cns.search.SeasonalRecipeDto> findSeasonalByExactTitles(@Param("titles") List<String> titles);
